...
const StatsDashboard: React.FC<StatsDashboardProps> = ({ players, matches, history, rackets, onUpdateRacket }) => {
  ...
  const [compareGameType, setCompareGameType] = useState<'singles' | 'doubles'>('singles');
  ...
  const compareData = useMemo(() => {
    ...
    // Head to Head - filtered by game type
    const h2hMatches = matches.filter(m =>
      m.type === compareGameType &&
    ...
    // Chart Data Construction - filtered by game type
    // 1. Get all relevant history entries for the selected game type
    const historyA = history.filter(h => h.playerId === primaryId && h.gameType === compareGameType);
    const historyB = history.filter(h => h.playerId === secondaryId && h.gameType === compareGameType);
  ...
  }, [primaryId, secondaryId, matches, history, compareGameType]);
  ...
  const getPrimaryElo = () => compareGameType === 'singles' ? primaryPlayer?.eloSingles : primaryPlayer?.eloDoubles;
  const getSecondaryElo = () => compareGameType === 'singles' ? secondaryPlayer?.eloSingles : secondaryPlayer?.eloDoubles;

  // Get game-type-specific stats
  const getPrimaryStats = () => {
    if (!primaryPlayer) return { wins: 0, losses: 0, streak: 0 };
    return {
      wins: compareGameType === 'singles' ? primaryPlayer.winsSingles : primaryPlayer.winsDoubles,
      losses: compareGameType === 'singles' ? primaryPlayer.lossesSingles : primaryPlayer.lossesDoubles,
      streak: compareGameType === 'singles' ? primaryPlayer.streakSingles : primaryPlayer.streakDoubles,
    };
  };

  const getSecondaryStats = () => {
    if (!secondaryPlayer) return { wins: 0, losses: 0, streak: 0 };
    return {
      wins: compareGameType === 'singles' ? secondaryPlayer.winsSingles : secondaryPlayer.winsDoubles,
      losses: compareGameType === 'singles' ? secondaryPlayer.lossesSingles : secondaryPlayer.lossesDoubles,
      streak: compareGameType === 'singles' ? secondaryPlayer.streakSingles : secondaryPlayer.streakDoubles,
    };
  };
            ...
            <div className="glass-panel rounded-xl overflow-hidden">
                <table className="w-full text-center">
                    ...
                    <tbody className="divide-y divide-white/5 font-mono text-sm">
                        <ComparisonRow
                            label={`${compareGameType === 'singles' ? 'Doubles' : 'Singles'} Elo`}
                            valA={compareGameType === 'singles' ? primaryPlayer.eloDoubles : primaryPlayer.eloSingles}
                            valB={compareGameType === 'singles' ? secondaryPlayer.eloDoubles : secondaryPlayer.eloSingles}
                        />
                         <ComparisonRow
                            label="Win Rate"
                            valA={`${primaryStats.wins + primaryStats.losses > 0 ? Math.round((primaryStats.wins / (primaryStats.wins + primaryStats.losses)) * 100) : 0}%`}
                            valB={`${secondaryStats.wins + secondaryStats.losses > 0 ? Math.round((secondaryStats.wins / (secondaryStats.wins + secondaryStats.losses)) * 100) : 0}%`}
                            highlight
                        />
                        <ComparisonRow
                            label="Current Streak"
                            valA={primaryStats.streak}
                            valB={secondaryStats.streak}
                        />
                        <ComparisonRow
                            label="Total Wins"
                            valA={primaryStats.wins}
                            valB={secondaryStats.wins}
                        />
                    </tbody>
                </table>
            </div>
